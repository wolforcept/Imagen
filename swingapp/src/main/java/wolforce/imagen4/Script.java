...
public class Script {
    ...
    public static String tryGetOrCreateScript() {
        File file = new File("script.kt");
    ...
    public List<String> run(Imagen imagen, DataGrid dataGrid, int rowIndex) {
        ...
        try {
            ...
            StringBuilder callStringBuilder = new StringBuilder("import wolforce.imagen4.Imagen\nScript(imagen, ");
            for (int i = 0; i < paramsNumber; i++) {
                String toAppend;
                ParamType paramType = paramsTypes[i];
                String paramName = paramsNames[i];
                String paramValue = dataGrid.get(rowIndex, i);
            ...
            scriptString = scriptString.replaceFirst("(?ms)interface Imagen \\{.*?}", "import wolforce.imagen4.Imagen");
