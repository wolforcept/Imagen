...
import static imagegenerator.engine.Util.drawTextWithImgs;
...
public class Blitzsphere extends Template {
...
//	private static final Font FONT_TEXT = new Font("Mythica Blitzsphere", Font.BOLD, 40);
	...
	static {
		IMG_ABREVS.put("am", "symbol_small_melee");
		IMG_ABREVS.put("ar", "symbol_small_ranged");
		IMG_ABREVS.put("ag", "symbol_small_magic");
		IMG_ABREVS.put("amm", "symbol_medium_melee");
		IMG_ABREVS.put("arr", "symbol_medium_ranged");
		IMG_ABREVS.put("agg", "symbol_medium_magic");
		...
		IMG_ABREVS.put("w", "symbol_small_wilds");
		IMG_ABREVS.put("c", "symbol_small_chaos");
		IMG_ABREVS.put("f", "symbol_small_faith");
		IMG_ABREVS.put("t", "symbol_small_time");
		IMG_ABREVS.put("e", "symbol_small_energy");
		IMG_ABREVS.put("o", "symbol_small_occult");
		IMG_ABREVS.put("0", "symbol_small_wanderer");
		...
		IMG_ABREVS.put("b", "symbol_small_blood");
		IMG_ABREVS.put("ww", "symbol_medium_wilds");
		IMG_ABREVS.put("cc", "symbol_medium_chaos");
		IMG_ABREVS.put("ff", "symbol_medium_faith");
		IMG_ABREVS.put("tt", "symbol_medium_time");
		IMG_ABREVS.put("ee", "symbol_medium_energy");
		IMG_ABREVS.put("oo", "symbol_medium_occult");
		IMG_ABREVS.put("00", "symbol_medium_wanderer");
	}

	public enum Type {
		NORMAL, ENEMY, LAND, RULES
	}

	private Type cardType;

	public Blitzsphere(Type cardType) {
		super(748, 1042);
	...
	public boolean lineValid(String line) {
		...
		return parts == 1 || parts == 5 || parts == 6 || parts == 7;
	}

	public void draw(Renderer r, String runName, String rulesText) {
		r.drawImage("back_rules", -34, -34);

		if (cardType != Type.RULES)
			return;

		r.setColor(COLOR_TEXT);
		r.setFont(FONT_TEXT);

		Util.drawTextWithWrap(r, 50, 88, FONT_TEXT, rulesText.split("\\|"), 51, 42, 628);

	}

	public void draw(Renderer r, String runName, String imgNr, String name, String type, String artist, String costsStr) {
		draw(r, runName, imgNr, name, type, artist, costsStr, "");
	}

	public void draw(Renderer r, String runName, String imgNr, String name, String type, String artist, String costsStr, String text) {

		System.out.println(imgNr);
		if (runName == "starterdeck") // when in starterdeck, set correct name of card type for each card
			runName = type.substring(0, type.indexOf(" ")).toLowerCase();
//		String runName = getFromImgChar(imgNr.charAt(0));
		...
		r.drawImage("back_" + runName, x0, y0);
		if (cardType == Type.NORMAL)
			r.drawImage("banner_" + runName, x0, y0);
		x = x0 + 62;
		y = y0 + 45;
		r.drawImage("symbol_medium_" + runName, x, y);
		x = x0 + 165;
		y = y0 + 93;
		...
		// WATERMARK
		if (cardType == Type.NORMAL && !runName.equals("wanderer")) {
...
//			y = y0 + 572;
		...
		r.drawString(type, x, y - 26);
		r.drawString(type, x, y - 26);
		r.setFont(FONT_TEXT_VERY_SMALL);
		r.drawString(trademark + "Art by " + artist, x, y);
			...
			if (costs.size() > 0) {
				for (int i = 0; i < costs.size(); i++) {
					...
					try {
						...
						String img = b ? "00" : "a" + c.charAt(c.length() - 1) + c.charAt(c.length() - 1);
